...
const Manager_view_expert = ({ expert, refresh_expert }) => {
    ...
    const handle_add_expertise = async () => {
        ...
        try {
            const response = await fetch(`${api_base_url}/api/add-expertise`, {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    expertise_ids: selected_tags.map((tag) => tag.value),
    ...
    const handle_remove_expertise = async () => {
        ...
        try {
            const response = await fetch(`${api_base_url}/api/remove-expertise`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRF-TOKEN": csrfToken,
    ...
    return (
        <div className="p-4 border rounded shadow-md" aria-labelledby="expert-name">
            <h3 className="text-xl font-bold" id="expert-name">
                {`${expert.First_name} ${
                    expert.Middle_name && expert.Middle_name !== "" ? expert.Middle_name + " " : ""
                }${expert.Surname}`}
            </h3>
            ...
            <div className="mt-4">
                ...
                <button
                    onClick={handle_remove_expertise}
