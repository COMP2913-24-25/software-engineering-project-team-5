...
const HomePage = () => {
    ...
    const nextPage = (type) => {
        if (type == "verified") {
            set_verified_index((prev) => (prev + items_per_page < verified_items.length ? prev + items_per_page : prev));
        } else {
            set_unverified_index((prev) => (prev + items_per_page < unverified_items.length ? prev + items_per_page : prev));
        }
    };
    ...
    const prevPage = (type) => {
        if (type == "verified") {
            set_verified_index((prev) => (prev - items_per_page >= 0 ? prev - items_per_page : prev));
        } else {
            set_unverified_index((prev) => (prev - items_per_page >= 0 ? prev - items_per_page : prev));
        }
    };
    ...
    return ( 
        <div className="container mx-auto p-8 text-center">
                <div className="text-center mb-8">
                    ...
                    <p className="text-lg text-gray-600">Bid and auction items efficiently.</p>
                </div>

                <h2 className="text-2xl font-semibold mb-4">Products</h2>
                <div className="flex items-center justify-center gap-4">
                    <ChevronLeft className="cursor-pointer" onClick={() => prevPage("unverified")}/>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                        {unverified_items.slice(unverified_index, unverified_index + items_per_page).map((item, index) => (
                            <Listing_item key={index} item={item} />
                        ))}
                    </div>
                    <ChevronRight className="cursor-pointer" onClick={() => nextPage("unverified")} />
                </div>
                    

                <h2 className="text-2xl font-semibold mb-4">SHOP AUTHENTIC</h2>
                <div className="flex items-center justify-center gap-4">
                <ChevronLeft className="cursor-pointer" onClick={() => prevPage("verified")}/>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                        {verified_items.slice(verified_index, verified_index + items_per_page).map((item, index) => (
                            <Listing_item key={index} item={item} />
                        ))}
                    </div>
                    <ChevronRight className="cursor-pointer" onClick={() => nextPage("verified")} />
                </div>
        </div>
    );
};
