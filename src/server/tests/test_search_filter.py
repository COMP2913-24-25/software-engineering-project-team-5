...
def test_get_search_filter_no_query(client, logged_in_user):
    ...
    response = client.post(
        "/api/get_search_filter", json={"item": True, "user": False, "searchQuery": ""}
    )
...
def test_get_search_filter_with_query(client, logged_in_user):
    response = client.post(
        "/api/get_search_filter", json={"item": True, "searchQuery": "Smart"}
    )
    assert response.status_code == 200
    data = json.loads(response.data)
    assert len(data) > 0
...
def test_get_search_filter_no_results(client, logged_in_user):
    response = client.post(
        "/api/get_search_filter", json={"item": True, "searchQuery": "NonExistentItem"}
    )
    assert response.status_code == 200
    data = json.loads(response.data)
    assert len(data) == 0
...
def test_get_search_filter_invalid_request(client, logged_in_user):
    response = client.post(
        "/api/get_search_filter",
        json={"user": True, "item": False, "searchQuery": "John"},
    )
    assert (
        response.status_code == 200
    )  # Should handle gracefully even if user search is not implemented
